...
export const getRestaurantEvents = () => async (dispatch) => {
  try {
    const res = await axios.get(
      `http://54.215.250.62:3002/restaurant/events/me`
    );
    dispatch({
      type: GET_EVENTS,
      payload: res.data,
...
export const getAllEvents = () => async (dispatch) => {
  ...
  try {
    const res = await axios.get(`http://54.215.250.62:3002/customer/events`);
    dispatch({
      type: GET_EVENTS,
      payload: res.data,
    });
...
export const getEventDetail = (Event_Name) => async (dispatch) => {
  ...
  try {
    const res = await axios.get(
      `http://54.215.250.62:3002/customer/events/eventdetail/${Event_Name}`
    );
    dispatch({
      type: GET_EVENT,
      payload: res.data,
...
export const updateEventByName = (
  ...
  try {
    ...
    const res = await axios.post(
      `http://54.215.250.62:3002/restaurant/events/updateevent/${Event_Name}`,
      formData,
      config
    );
    dispatch({
