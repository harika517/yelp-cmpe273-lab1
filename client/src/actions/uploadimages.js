...
export const insertImage = (image, Cust_Email) => async(dispatch) => {
    try {
        ...
        const res = await axios
            .post(`http://54.215.250.62:3001/customer/addphoto/`, formData, config)
            .then((response) => {
...
export const insertRestImage = (image, Rest_email_id) => async(dispatch) => {
    try {
        ...
        const res = await axios
            .post(`http://54.215.250.62:3001/restaurant/addphoto/`, formData, config)
            .then((response) => {
                alert('Image uploaded successfully');
                console.log('response is ', response);
            });
...
export const insertItemImage = (image, item_id) => async(dispatch) => {
    try {
        ...
        const res = await axios
            .post(
                `http://54.215.250.62:3001/item/addphoto/${item_id}`,
                formData,
                config
            )
            .then((response) => {
